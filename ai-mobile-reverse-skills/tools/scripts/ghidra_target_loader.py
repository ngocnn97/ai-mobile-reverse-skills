...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-activate",
        action="store_true",
        help="Launch the project but do not try to foreground Ghidra on macOS",
    )
...
def try_launch_gui(ghidra_run: Path, project_file: Path, activate: bool) -> dict:
    ...
    if activate and system == "darwin":
        for app_name in ("Ghidra",):
            try:
                subprocess.run(
                ...
                launch_meta["notes"].append(f"activate {app_name} failed: {exc}")
